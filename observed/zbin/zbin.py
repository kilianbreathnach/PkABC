...
def Pk_zslice(mock_name, i_mock, n_slices, **kwargs):
    ...
    * Calls spec from Spectrum module 
    ...
    for i_slice in range(1,n_slices+1):
        slice_corr_name = ''.join(['zbin', str(i_slice), 'of', str(n_slices)])
...
def Pk_zslice_mpwrap(params): 
    ''' Multiprocessing wrapper for Pk_zslice function
    '''
    mock_name = params[0]
    i_mock = params[1]
    n_slices = params[2]
    ...
    Pk_zslice(mock_name, i_mock, n_slices, **kwargs)
...
def Pk_zslice_mp(mock_name, n_mocks, n_slices, Nthreads=5, **kwargs):
    ...
    mapfn(Pk_zslice_mpwrap, [arg for arg in arglist])
...
if __name__=='__main__': 
    Pk_zslice_mp('nseries', 20, 5, Nthreads=10, Ngrid=360)
