...
def pmc_abc(data, 
    ...
    sub.hist2d(theta_t[0,:], theta_t[1,:], weights=w_t)
    ...
    while t < T: 
        ...
        for i in xrange(N): 
            ...
            theta_starstar = multivariate_normal( theta_star, sig_t_1 ).rvs(size=1)[0]
            ...
            while rho > eps_t: 
                ...
                theta_star = weighted_sampling( theta_t_1, w_t_1 ) 
                theta_starstar = multivariate_normal(theta_star, np.sqrt(sig_t_1)).rvs(size=1)[0]
                model_starstar = toolz.simulator( theta_starstar )
                rho = test_dist(data[1], model_starstar(data[0])) 

            #print theta_star, theta_starstar
            theta_t[i] = theta_starstar
            w_t[i] = toolz.prior().pdf(theta_t[i]) / np.sum(w_t_1 * multivariate_normal(theta_t[i], sig_t_1).pdf(theta_t_1))
